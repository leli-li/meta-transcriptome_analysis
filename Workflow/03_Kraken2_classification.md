...
# Download taxonomy files and accession-to-taxid mapping files
kraken2-build \
  --download-taxonomy \
...
# The --protein option indicates that the library contains amino acid sequences.
# The --no-masking option skips low-complexity masking during library addition.
kraken2-build \
...
# The --fast-build option was used because repeated standard builds did not complete within the available runtime.
# This option reduces multithreaded build time but does not guarantee deterministic database construction.
kraken2-build \
  --build \
  --db "/path/to/kraken2_database/nr_bacteria_and_fungi" \
  --protein \
...
# Ensure that the input filenames match the final output filenames documented in Section 02.1.
